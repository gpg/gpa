		       ...
		       gpointer calldata)
{
  if (file == NULL)
	...
	| GPAPA_FILE_STATUS_COMPRESSED | GPAPA_FILE_STATUS_LITERAL:
	  return (GPAPA_FILE_SIGNED);
	case GPAPA_FILE_STATUS_SIGNATURE | GPAPA_FILE_STATUS_LITERAL:
...
gpapa_file_encrypt_and_sign (GpapaFile * file, gchar * targetFileID,
			     ...
			     gchar * PassPhrase, GpapaSignType SignType,
...
gpapa_file_decrypt (GpapaFile * file, gchar * targetFileID,
		    gchar * PassPhrase, GpapaCallbackFunc callback,
		    gpointer calldata)
{
  if (file == NULL)
    callback (GPAPA_ACTION_ERROR, "missing file name", calldata);
